...
func check_guess():
	if guess == combination:
		$AudioStreamPlayer.stream = load("res://assets/Heist_Meisters_Assets/SFX/threeTone1.ogg")
		$AudioStreamPlayer.play()
		light.texture = load("res://assets/Heist_Meisters_Assets/GFX/Interface/PNG/dotGreen.png")
		$Timer.start()
	else:
		reset_lock()
	
func enter(button):
	$AudioStreamPlayer.stream = load("res://assets/Heist_Meisters_Assets/SFX/twoTone1.ogg")
	$AudioStreamPlayer.play()
	guess.append(button)
	update_display()
...
func reset_lock():
	light.texture = load("res://assets/Heist_Meisters_Assets/GFX/Interface/PNG/dotRed.png")
	display.text = ""
	guess.clear()
